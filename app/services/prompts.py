...
ANSWER_PROMPT_TEMPLATES = {
    ...
    "summary_query": (
        ...
        "    • 'What topics were discussed?' / 'What was on the agenda?' →\n"
        "      Start with: 'Here are the topics discussed in this meeting:'\n"
        "      Then a numbered list. For each topic use this exact structure:\n\n"
        "      **N. Bold Topic Name** (4-7 words, precise and descriptive)\n"
        "      One paragraph describing what was discussed, why it mattered, and the key outcome.\n"
        "      Then sub-bullets with specific attributed details — use natural attribution language:\n"
        "        - '[Speaker Name] raised:' or '[Speaker Name] explained:' or '[Speaker Name] asked:'\n"
        "        - '[Speaker Name] confirmed:' or '[Speaker Name] committed to:' or 'Agreement:'\n"
        "        - 'The team decided:' or 'Open question:' if no individual speaker is identifiable\n"
        "      NEVER write 'Raised by: Meeting Summary' or 'Raised by: (unknown)' — these are\n"
        "      forbidden. If you cannot find a real speaker name in the content, write 'The team' or\n"
    ...
    "topic_summary_query": (
        ...
        "    • If YES — organize your answer by KEY THEMES or discussion points (NOT by speaker).\n"
        "      For each theme use a bold header, then sub-bullets covering:\n"
        "        – what was debated or clarified under that theme\n"
        "        – who said what (speaker + timestamp) and what they meant\n"
        "        – the conclusion or decision reached for that theme (if any)\n"
        "      After all themes: if specific items were EXPLICITLY left unresolved in the\n"
        "      transcript, add a brief 'Unresolved' section — but ONLY with a [n] citation\n"
        "      for each item. Do NOT infer or speculate; skip this section entirely if you\n"
        "      cannot point to a chunk that directly states something is unresolved.\n"
        ...
        "      Stop there. Do NOT add timestamped blocks or quotes from the transcript.\n\n"
        "    • If YES — start with: 'Here are the [topic]-related topics discussed in this meeting:'\n"
        "      Then a numbered list. For each sub-topic use this exact structure:\n\n"
        "      **N. Bold Sub-Topic Name** (4-7 words, precise)\n"
        "      One paragraph describing what was discussed and why it mattered.\n"
        "      Then sub-bullets with specific attributed details — use natural attribution language:\n"
        "        - '[Speaker Name] raised:' or '[Speaker Name] explained:' or '[Speaker Name] asked:'\n"
        "        - '[Speaker Name] confirmed:' or '[Speaker Name] committed to:' or 'Agreement:'\n"
        "        - 'The team decided:' or 'Open question:' if no individual speaker is identifiable\n"
        "      If a phase/scope distinction was made (e.g. Phase A vs Phase B), include it.\n"
        "      End each topic with any agreements, decisions, or next steps reached.\n\n"
        ...
        "- Do NOT add 'Open Questions', 'Key Takeaways', or 'Conclusion' sections unless\n"
        "  you can back each item with a direct [n] citation from the context chunks.\n"
        + _TIMESTAMP_RULE
        + _CITATION_RULE +
        "\nContext from meeting transcripts:\n{context}\n\n"
        "Question: {query}\n\nAnswer:"
