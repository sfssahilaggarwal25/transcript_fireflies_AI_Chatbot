...
HOW TO RUN
----------
# Run all goldens (all 3 categories):
python -m app.tests.deepeval_tests.run_deepeval --project-id proj_nolocode_001

# Run one category only:
python -m app.tests.deepeval_tests.run_deepeval --project-id proj_nolocode_001 --category meeting
python -m app.tests.deepeval_tests.run_deepeval --project-id proj_nolocode_001 --category project
python -m app.tests.deepeval_tests.run_deepeval --project-id proj_nolocode_001 --category topic

# Run specific goldens by ID:
python -m app.tests.deepeval_tests.run_deepeval --project-id proj_nolocode_001 --ids A1,B2,C3

# Dry run (show what would run without calling the pipeline):
python -m app.tests.deepeval_tests.run_deepeval --project-id proj_nolocode_001 --dry-run
"""
...
from app.rag.tests.deepeval_tests.goldens import ALL_QUERIES, MEETING_QUERIES, PROJECT_QUERIES, TOPIC_QUERIES
...
def filter_goldens(goldens, ids: list[str] | None):
    """Filter to specific golden IDs if --ids was passed."""
    if not ids:
        return goldens
    id_set = set(ids)
    filtered = [
        g for g in goldens
        if (g.additional_metadata or {}).get("id") in id_set
    ]
    missing = id_set - {(g.additional_metadata or {}).get("id") for g in filtered}
    if missing:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated golden IDs to run (e.g. --ids A1,B2,C3)",
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    goldens   = filter_goldens(dataset.goldens, [i.strip() for i in args.ids.split(",")] if args.ids else None)
